...
var app = function(window){
	...
	var
				...
				*
				*/
		   userId = global_get( 'id' )
			,unit = global_get( 'unit' ) || 1
			,click_macro  = global_get( 'click_macro' )
	...
	return {
		...
		initial_setup: function(window){
			...
			*/
			...
			$.ajax({
			    type: "GET",
			    url: '/user/' + userId + '/deploy/data/global.xml?random=' + Math.floor(Math.random()*999999999),
			    dataType: "xml",
					success: function(xml) {	

						custom_tab_title = $(xml).find('tab').text();

						var  size
						    //,controllers = app.navigational_icon_layout(xml);
						    //,controllers = veneer.navigational_icon_layout();
						    
//						appHasMusic = ( typeof(controllers) != "undefined" ? app.inArray('music', veneer.controllers):false );
						    
//						veneer.setHomeScreenImage();						    
//						veneer.logo_avatar_header_banner();
//						veneer.layout();
						...
						*/
						...
						abouttitle = ( $(xml).find('abouttitle').text() !='' ? $(xml).find('abouttitle').text():$(xml).find('name').text())
						facebookID = $(xml).find('facebookID').text();
						twitter = $(xml).find('twitter').text();
						hashtag = $(xml).find('hashtag').text();
						channelid = $(xml).find('channelid').text();
						ustreamid = $(xml).find('ustreamid').text();
//						base_url = $(xml).find('base_url').text();
//						base_url2 = $(xml).find('base_url2').text();
						custom_html = $(xml).find('custom_html').text();
						
						// GLOBAL
						gid = $(xml).find('gid').text();  // GOOGLE ANALYTICS			
													
						app.url2ShareBecomes();
							
						$(document).attr("title", 
							abouttitle.toLowerCase().replace(/\b[a-z]/g, function(letter) {
							    return letter.toUpperCase();
							})						
						);
	
						$('#liveStreanLink').css({cursor:'pointer'}).click(function(event) {
									app.displayUstream();
						});	
						
						app.music_player_setup(veneer.controllers);
						app.adjustShareOverlay();
			...
			var  unit_url = ( click_macro && click_macro != '' ?  unescape(click_macro) + unescape(base_url) : base_url )			// homescreen & avatar
					,unit_url2 = ( click_macro && click_macro != '' ?  unescape(click_macro) + unescape(base_url2) : base_url2 );  // top banner
			...
			$('#homePageContainer div img').live('click', function(event) {
			...
			$('.overlayNavItem').css({
						'width': veneer.format[unit].navMenuverlay.width
						});
			$('.elementsWrapper').css({
						'margin-top': veneer.format[unit].elementsWrapper.marginTop,
						height: veneer.format[unit].elementsWrapper.height
						});
			$('.elementsInnerWrapper').css({'width': veneer.format[unit].elementsInnerWrapper.width});
							
							
			
			$('.panels').width(veneer.format[unit].width);	
...
//		,navigational_icon_layout:function(xml){
//			
...
//							}else{
//								tab_name = custom_tab_title;
//								var icon_image = '/user/' + userId + '/deploy/userassets/tab_icon';
//							};
//	
//							controllers.push (controller);
//	
//							var new_panel_div = '\
//							<div  class="iconbox"   style="background:transparent; "    controller="' + controller + '">\
...
//									
//									$(this).width(veneer.format[unit].nav_icon_size)
//						});	
//						
//						icons = '';
//						
...
//							}else{
//								tab_name = custom_tab_title;
//								icon_image = '/user/' + userId + '/deploy/userassets/tab_icon';
//							};
//							
...
//		}		
		...
		*
		*/
		,adjustShareOverlay:function(){
			
						if( app.BrowserDetect.browser == 'Firefox' ){
							$('#socialWrapper').css({right:'40px'})
						};
						
						if( unit == 3 ){// INTERACTIVE AD UNIT
							
								$('#socialWrapper table td.first-row').height(veneer.format[unit].height-165);
								...
								switch(app.BrowserDetect.browser){ 
									case 'Firefox': 
											$('#socialWrapper table td.first-row').height(veneer.format[unit].height-245);
									break;
									case 'Chrome': 
											$('#socialWrapper table td.first-row').height(veneer.format[unit].height-220);
									break;							
									case 'Explorer': 
											$('#socialWrapper table td.first-row').height(veneer.format[unit].height-215);
									break;							
								};		
		...
		*/
		,url2ShareBecomes:function(){
						...
						if( unit  == 1){ 
							urlToShare = window.location.hostname + "/user/" + userId + "/deploy/embed/400x300.html";
						}else{
							urlToShare = window.location.hostname + "/user/" + userId + "/deploy/embed/" +  size  + ".html";
						};
		}
		,music_player_setup:function(controllers){
						if(    app.BrowserDetect.browser == 'Firefox'  // need to check if this is true
		...
		,googleAnalytics: function( arg ){
				switch(arg.type){
							case "events": {
								...
								_gaq.push(['_trackEvent', arg.category, arg.action, userId + '_' + arg.label]);
							};	break;
							case "pageviews": {
								...
								_gaq.push(['_trackPageview', '/unit/' + userId + '/' + arg.section  + '/' +  arg.unique_desc]);
							};	break;
				}	
 		}
		,scrollTo: function(){
		...
		,scroll:function(){
						...
						$(".belt").animate({left: "+=" + veneer.format[unit].nav_icon_size}, 100);	
		...
		,displayShareThis:function(){
			...
			/* 
					...
					switch(unit){
			//			case "1": unit = 1;	break; // Mobile
			//			case "2": unit = 2;	break; // Web Player
						case "3": width =  break; // Interactive Ad Unit
			//			case "4": unit = 4;	break; // Mini-Website
					}		
		...
		,displayUstream:function(){
						...
						if ($('.ustreamOverlay').css('display') == 'none') { 
							...
							var ustream = '\
							<iframe  id="ustream_iframe"  src="http://www.ustream.tv/embed/' + channelid  + '" \
							width="' + veneer.format[unit].width + '" height="' + (veneer.format[unit].height - 92)  + '" scrolling="no" frameborder="0" \
		...
		,adjustScrollPanel:function(visibility){
					...
					$('.scroll-pane-arrows').height(veneer.format[unit].scrollPaneAarrows);
		...
		,adjust_slideUpDownHeight:function(){
					$('.slideUpDown').css({'overflow-y':'auto'}).height(veneer.format[unit].height - veneer.format[unit].panel_height_unit);
		}			
		,setup_marquee:function(){
					
						$('.scrollDisplay').width(veneer.format[unit].scrollDisplay.width).css({'margin-left':veneer.format[unit].scrollDisplay.marginLeft})
		...
		*/
		,setup_music_player:function(){
						...
						$.ajax({
						    type: "GET",
						    url:  '/user/' + userId + '/deploy/data/music.xml',
						    dataType: "xml",
								success: function(xml) {	
		...
		,create_embed_code_for_sharing:function(){
						...
						* OLD FLASH EMBED
						var embed_code = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="400" height="350" id="widget400x350" align="middle">\
							<param name="movie" value="/flash/widget400x350.swf?artistid=' + userId   + '" />\
							...
							<embed src="/flash/widget400x350.swf?artistid="' +  userId   + '" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="400" height="350"></embed>\
							</object>\
						';						
						...
						*/ 
						...
						if( unit  == 1){
							var embed_code = '<script type="text/javascript" language="Javascript" src="http://cdn.republicproject.com/user/' +  userId  + '/deploy/embed/400x300.js"></script>';
						}else{
							var embed_code = '<script type="text/javascript" language="Javascript" src="http://cdn.republicproject.com/user/' +  userId  + '/deploy/embed/' + size + '.js"></script>';
						};
						...
						$('#twitterShareBtn').click(function(){
								...
								var link = 'http://twitter.com/intent/tweet?text=' + encodeURIComponent( window.hashtag + ' http://' + urlToShare); 
		...
		*/
		...
		,showvideos:function(){
			...
			$.ajax({
			    type: "GET",
			    url: '/user/' + userId + '/deploy/data/videos.xml?random=' + Math.floor(Math.random()*999999999),
			    dataType: "xml",
					success: function(xml) {	
									...
									<div class="videoInnerItemHolder navLink "   youtube_id="' +  youtube_id  + '" controller="videoDetails" >\
										<div   class="video_item_left" >\
												...
												<div class="videoLabelWrapper"  >\
														<div class="videoLabel" style="font-size: ' + veneer.format[unit].font_size_unit.big + 'px;text-overflow: ellipsis;">\
															' + title + '\
														</div>\
														<div class="albumInfo" style="font-size: ' + veneer.format[unit].font_size_unit.small + 'px;text-overflow: ellipsis;">\
															' + description + '\
														</div>\
												</div>\
										</div>\
		...
		,showvideosdetails:function(){
					...
					switch(unit){
						case "1": { // Mobile
										...
										$('#iframe_video').height(veneer.format[unit].height - 196);
		...
		,show_merch: function(){
			...
			$.ajax({
			    type: "GET",
			    url: '/user/' + userId + '/deploy/data/packages.xml?random=' + Math.floor(Math.random()*999999999),
			    dataType: "xml",
					success: function(xml) {	
																				...
																				<div class="merchItemHolder navLink "  merchId=' + id   + '  controller="merchDetails"   style="cursor:pointer;"  >\
																						<div   style="width:90%;float: left;white-space:nowrap" >\
																								...
																								<div class="merchLabelHolder"   style="float: left;"  >\
																										<div class="merchNameContainer" style="font-size: ' + veneer.format[unit].font_size_unit.big + 'px;clear:both;text-overflow: ellipsis;">\
																											' +   title + '\
																										</div>\
																										<div class="priceNameContainer" style="font-size: ' + veneer.format[unit].font_size_unit.small + 'px;text-overflow: ellipsis;">\
																											Price: ' +  price  + '\
																										</div>\
																								</div>\
																						</div>\
						...
						$('.merchLabelHolder').css({width:veneer.format[unit].width - 150})							
					...
					success: function(xml) {	
						...
						$(xml).find('item').each(function(){
												...
												if(id == window.merchId_selected ){
															...
															$('#title_div').css({'font-size':veneer.format[unit].font_size_unit.big}).text(title)
		...
		,show_theaters:function(){
			...
			switch(unit){
				...
				case "4": { // Mini-Website
					$('#theaters').css({
							height:veneer.format[unit].height-123
					});				
				};	break; 
			};
				    	...
				    	<div  class='theater_names_wrapper play_arrow'    theater-num='" + theater + "'   style='cursor:pointer;border-bottom:1px solid black'    >\
				    			<div   style='float:left;width:92%'  >\
							    	...
							    	<div  class='theater_names_right_wrapper ' >\
												    	<div  class='theater_names '    style='font-size: " + veneer.format[unit].font_size_unit.big + "px;'    >\
													    	" + theaters[theater].name + "\
												    	</div>\
												    	<div  class='theater_address '    style='font-size: " + veneer.format[unit].font_size_unit.small + "px;'     >\
...
//				    
		...
		,showmusic: function(){
			...
			$.ajax({
			    type: "GET",
			    url:  '/user/' + userId + '/deploy/data/music.xml?random=' + Math.floor(Math.random()*999999999),
			    dataType: "xml",
					success: function(xml) {	
													...
													<div class="musicItemContainer">\
															<!--  --><div   style="width:65%;float: left;white-space:nowrap" >\
																...
																<span class="musicLabelHolder" style="float: left;display: inline-block ">\
																		<div class="songNameContainer navLink"  musicId='+musicId+' controller="musicDetails" style="cursor:pointer;font-size: ' + veneer.format[unit].font_size_unit.big + 'px;  text-overflow: ellipsis;">'+ songname +'</div>\
																		<div class="albumNameContainer navLink"  musicId='+musicId+' controller="musicDetails" style="cursor:pointer;font-size: ' + veneer.format[unit].font_size_unit.small + 'px;text-overflow: ellipsis;">' + albumname +  '</div>\
																</span>\
															<!--  --></div>\
						...
						$('.musicLabelHolder, .musicLabelHolder div').width(veneer.format[unit].width-200);
		...
		,showphotos: function(){
			...
			$.ajax({
			    type: "GET",
			    url: '/user/' + userId + '/deploy/data/photos.xml?random=' + Math.floor(Math.random()*999999999),
			    dataType: "xml",
					success: function(xml) {	
		...
		,showphotodetail: function(){
					...
					success: function(xml) {	
						...
						$(xml).find('item').each(function(){
							...
							if(photoid == window.photoId_selected ){
								...
								$('#image_detail').width(veneer.format[unit].width).attr('src',url)		
			...
			$('#inside_content').css({'overflow-y':'auto'}).height(veneer.format[unit].height - veneer.format[unit].panel_height_unit);
...
//			};	
			...
			//$('.slideUpDown').css({'overflow-y':'auto'}).height(window.widget_height - app.panel_height_connect_tab_unit[unit][isSlideDown]);
			    ...
			    error:function(data) {
					...
					success: function(data) {
							...
							*/
							...
							<div class="feedItemContainer">\
								...
								<div class="addOverflowHidden addMargin10">\
									<div class="twitterContent"   style="font-size: ' + veneer.format[unit].font_size_unit.big + 'px;text-align:left;color:#CCCCCC"  ><small>' +  tweeted_text  + '</small></div>\
									<div class="twitterContentTimeStamp addFontSize10"   style="font-size: ' + veneer.format[unit].font_size_unit.small + 'px;text-align:left;"  >' +  created_formated  + '</div>\
				  	...
				  	$('#feed').linkify();	
						...
						$('.scroll-pane-arrows').height(veneer.format[unit].scrollPaneArrowsConnectTabUnit);
							    ...
							    created_ = new Date(created)
							   	created_formated = created_.format("dddd, mmmm dS, yyyy, h:MM:ss TT");					    
									...
									<div class="feedItemContainer">\
										...
										<div class="addOverflowHidden addMargin10">\
											<div class="twitterContent"  style="font-size: ' + veneer.format[unit].font_size_unit.big + 'px;text-align:left;color:#CCCCCC"  ><small>' +  tweeted_text  + '</small></div>\
											<div class="twitterContentTimeStamp addFontSize10" style="font-size: ' + veneer.format[unit].font_size_unit.small + 'px;text-align:left;color:#CCCCCC" >' +  created_formated  + '</div>\
...
//							});
		      		...
		      		// console.log( $('#oneFeed').html());		
							...
							var feed_segment = '\
							<div class="feedItemContainer"    style="padding:5px;height:auto;font-size: ' + veneer.format[unit].font_size_unit.big + 'px;margin-left:10px;text-align:left;color:#CCCCCC"   >\
								<div  class="image_container"   >' +  ( oneImageWithWrapper || '' ) + '</div>\
								<div   style="font-size:11px"  >' +   $('#oneFeed').html() + '</small></div>\
							</div>\
							<div   style="padding-left:5px;padding-top:8px;clear:both;font-size: ' + veneer.format[unit].font_size_unit.small + 'px;margin-left:10px;text-align:left;color:#CCCCCC;margin-bottom:5px"  >' + published_formated  + '\
							</div>\
		...
		,show_events: function(find_zipcode){
			var month_textualized = new Array(12);
			  ...
			  month_textualized[12] = 'Dec';
				...
				$.ajax({
			    type: "GET",
			    url: '/user/' + userId + '/deploy/data/events.xml?random=' + Math.floor(Math.random()*999999999),
			    dataType: "xml",
					success: function(xml) {	
													...
													<div class="eventsItemContainer  ">\
														<!--  --><div   style="width:90%;float: left;white-space:nowrap" >\
																...
																<span class="eventsLabelHolder"  style="float: left;">\
																		<div class="eventsNameContainer" style="font-size: ' + veneer.format[unit].font_size_unit.big + 'px;text-overflow: ellipsis;">\
																			' +  eventname + '\
																		</div>\
																		<div class="dateNameContainer" style="text-overflow: ellipsis;">\
																			<span style="font-size: ' + veneer.format[unit].font_size_unit.small + 'px;color: #51C4FF;">' +  location  + '</span>\
																		</div>\
																 </span>\
														<!--  --></div>\
						...
						$('.eventsLabelHolder div').width(veneer.format[unit].width-180);
		...
		,showeventsdetails: function(){
					...
					success: function(xml) {	
						...
						$(xml).find('item').each(function(){
							...
							if(eventsid == window.eventsId_selected ){
								...
								$('#eventname').css({'font-size':veneer.format[unit].font_size_unit.big}).text(eventname);
								$('#datetime').css({'font-size':veneer.format[unit].font_size_unit.big}).text(datetime);
								$('#location').css({'font-size':veneer.format[unit].font_size_unit.big}).text(location);
								$('#eventinfo').css({'font-size':veneer.format[unit].font_size_unit.big}).text(eventinfo);
			...
			$('#right_content').css({width:veneer.format[unit].width - 157});
		...
		,showbio: function(){
			...
			$('.bioContent').text(about);
			$('.bioHeader').text(abouttitle);
		...
		,shownews: function(){
			...
			$.ajax({
			    type: "GET",
			    url: '/user/' + userId + '/deploy/data/news.xml?random=' + Math.floor(Math.random()*999999999),
			    dataType: "xml",
					success: function(xml) {	
		...
		,showcustom: function(){
			$('.subHeaderTitle').text(custom_tab_title);
